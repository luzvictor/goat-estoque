...
import { MoreHorizontal, Loader2, ChevronsUpDown, Check, PlusCircle } from "lucide-react";
...
export default function ProdutosPageClient() {
  ...
  async function criarProduto() {
    ...
    try {
        ...
        if (response.ok) {
          ...
          alert("Erro ao criar o produto: " + errorData.error);
        }
    } catch(error) {
  ...
  async function handleDeleteVariant(id_variante: string) {
    if (!window.confirm("Tem certeza que deseja excluir esta variante?")) return;
    const response = await fetch(`/api/variantes/${id_variante}`, { method: "DELETE" });
    if (response.ok) {
      await fetchProdutos();
    } else {
      const errorData = await response.json();
  ...
  async function handleRegisterEntry() {
    if (!entryForm.varianteId) { alert("Por favor, selecione uma variante."); return; }
    if (!entryForm.quantidade || entryForm.quantidade <= 0) { alert("Por favor, insira uma quantidade válida."); return; }
    setIsLoading(true);
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        alert(`Erro ao registrar entrada: ${errorData.error}`);
      }
    } catch (error) {
      alert("Ocorreu um erro de rede.");
    } finally {
                        ...
                        <PopoverContent className="w-auto p-2" align="end">
                          <div className="flex flex-col gap-2">
                            ...
                            <Button
                              ...
                              size="sm"
                              className="w-full justify-start"
                              onClick={() => handleDeleteVariant(produto.id_variante)}
                            >
                              Excluir Variante
                            </Button>
                          </div>
